...
//    private static final String DEFAULT_POOL_NAME = "default";
    ...
    public static File createGenesisTxnFile(String filename,int type,Context context) throws IOException {
        ...
        writeTransactions(file,type,context);
        return file;
    }

    public static void writeTransactions(File file, int type, Context context) throws IOException {
        ...
        String defaultTxns = LedgerNetworkType.getTransactionV2(context,type);

        FileWriter fw = new FileWriter(file);
//        for (String defaultTxn : defaultTxns) {
            fw.write(defaultTxns);
//            fw.write("\n");
//        }
    ...
    public static String createPoolLedgerConfig(int type,Context context) throws InterruptedException, ExecutionException, IndyException, IOException {
        Pool.deletePoolLedgerConfig(LedgerNetworkType.getConfigName());
        createPoolLedgerConfig(LedgerNetworkType.getConfigName(),type,context);
        return LedgerNetworkType.getConfigName();
    }

    private static void createPoolLedgerConfig(String poolName,int type,Context context) throws IOException {
        File genesisTxnFile = createGenesisTxnFile(LedgerNetworkType.getFileName(),type,context);
    ...
    public static Pool createAndOpenPoolLedger(int type,Context context) throws IndyException, InterruptedException, ExecutionException, IOException {
        String poolName = PoolUtils.createPoolLedgerConfig(type,context);
